...
## 2026-04-08 — Electron preload compiled as ESM → window.electronAPI undefined
What happened: Desktop package has `"type": "module"`, so `preload.ts` compiled to ESM (`import` statements). Electron's preload context doesn't reliably support ESM — `contextBridge.exposeInMainWorld` silently failed, leaving `window.electronAPI` undefined in the renderer.
Rule going forward: Always use `.cts` extension for the preload script. TypeScript NodeNext compiles `.cts` → `.cjs` (CommonJS) regardless of package type. Update the `preload:` path in BrowserWindow webPreferences to point at the `.cjs` file.
